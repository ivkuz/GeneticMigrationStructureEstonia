...
# Save sibling table
write.table(k, "~/tmp/siblings.tsv", row.names = F, quote = F, sep = "\t")
...
write.table(results, "~/tmp/results_family_sib_est.tsv", row.names = F, quote = F, sep = "\t")



# Calculate Variance explained by regions and the respective F-statistic
results <- fread("~/tmp/results_family_sib_est.tsv")
...
write.table(fstat_tab, opt$out, quote = F, sep = "\t", row.names = F)



###########################################################
# Calculate correlations between PRSs from sibling design #
###########################################################

# Fix k as k_orig
k_orig <- copy(k)
# table with future results
k_sibprs <- data.frame(vkood = unique(c(k$ID1, k$ID2)))

# Calculate PGS - mean_sib_PGS for each PRS
for(prs in prs_codes){
  ...
  ksib1 <- k[, .(ID1, prs1)]
  ksib2 <- k[, .(ID2, prs2)]
  colnames(ksib1) <- c("vkood", prs)
  colnames(ksib2) <- c("vkood", prs)
  ksib <- rbind(ksib1, ksib2)
  ksib <- unique(ksib)
  
  k_sibprs <- merge(k_sibprs, ksib, by = "vkood")
  
}  

k_sibprs <- as.data.table(k_sibprs)

# Calculate correlations
correlations <- data.frame()
for(prs_name in prs_codes){
  prs <- k_sibprs[, .SD, .SDcols=prs_name]
  cor_val <- cor(prs, k_sibprs$EA4)
  correlations <- rbind(correlations, c(prs_name, "EA4", cor_val))
}

# Save the results
colnames(correlations) <- c("Variable1", "Variable2", "cor")
correlations <- as.data.table(correlations)
write.table(correlations, "/gpfs/space/home/kuznetsi/EBB_project/analysis/prs_correlations_EA4_family_sib.tsv", row.names = F, quote = F, sep = "\t")
